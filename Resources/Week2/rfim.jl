...
```math
\mathcal{H}_N(\mathbf{S}, h) = - \frac{1}{2N} \sum_{i,j} S_i S_j - \sum_i h_i S_i
```
"""
...
```math
\phi_{RS}(m, \beta, \Delta) = -\frac{\beta m^2}{2} + \mathbb{E}_h \left[\log (2 \cosh(\beta(h+m))) \right]
```
with extrema $m^*(\beta, \Delta)$ that satisfy the fixed point equation
```math
m^* = \mathbb{E}_h \left[ \tanh(\beta(h + m^*)) \right]
```
"""
...
function fixed_point(m; β, Δ, tol)
	error = Inf
	while error > tol
		prob = IntegralProblem(integrand_fixed_point, -3Δ - 2, +3Δ + 2, (β, Δ, m))
		sol = solve(prob, QuadGKJL())
		m_new = sol.u
		error = abs(m_new - m)
		m = m_new
...
let
	β, Δ = 10^logβ, 10^logΔ
	m_grid = -1:0.01:1
	plot(m_grid, ϕ_RS.(m_grid; β, Δ), linewidth=2, label=nothing)
	m_opt = collect(fixed_points(; β, Δ, tol=1e-5))
	scatter!(m_opt, ϕ_RS.(m_opt; β, Δ), label=nothing)
	plot!(
		title="Potential function\n" *
			L"\beta = %$(round(β; digits=2)) \qquad \Delta = %$(round(Δ; digits=2))",
		xlabel=L"m",
		ylabel=L"\phi_{\mathrm{RS}}(m)"
	)
end
...
```math
\Phi_N = \frac{\log Z_N}{N}
```
and therefore
```math
\partial_\beta \Phi_N = \frac{1}{N} \frac{\partial_\beta Z_N}{Z_N} = \frac{1}{N} \sum_{\mathbf{S}} -\mathcal{H}_N(\mathbf{S}) \frac{\exp(-\beta \mathcal{H}_N(\mathbf{S}))}{Z_N} = - \frac{1}{N} \langle \mathcal{H}_N \rangle_\beta
```
"""
...
```math
\partial_2 \phi_{RS}(m, \beta, \Delta) = -\frac{m^2}{2} + \mathbb{E}_h \left[\frac{(h+m) 2\sinh(\beta(h+m))}{2 \cosh(\beta(h+m))}\right]
```
And we conclude
```math
e_{RS}(\beta, \Delta) = \frac{(m^*)^2}{2} - \mathbb{E}_h \left[(h+m^*) \tanh(\beta(h+m^*))\right]
```
"""
...
```math
\mathbb{P}_\beta \propto \exp(-\beta \mathcal{H}_N)
```
...
```math
S_i^{t+1} = \mathrm{sign} \left(h_i + \frac{1}{N} \sum_i S_i^t \right)
```
"""
...
```math
\begin{aligned}
m^* & = \mathbb{E}_h[\mathrm{sign}(h + m^*)] \\
& = -\int_{-\infty}^{-m^*} \frac{e^{-h^2/2\Delta}}{\sqrt{2\pi\Delta}} \mathrm{d}h + \int_{-m^*}^{+\infty} \frac{e^{-h^2/2\Delta}}{\sqrt{2\pi\Delta}} \mathrm{d}h \\
& = - \frac{1}{\sqrt{\pi}} \int_{-\infty}^{-m^*/\sqrt{2\Delta}} e^{-x^2}\mathrm{d}x + \frac{1}{\sqrt{\pi}} \int_{-m^*/\sqrt{2\Delta}}^{+\infty} e^{-x^2} \mathrm{d}x \\
& = - \left[\frac{\mathrm{erf}(x)}{2}\right]_{-\infty}^{-m^*/\sqrt{2\Delta}} + \left[\frac{\mathrm{erf}(x)}{2}\right]_{-m^*/\sqrt{2\Delta}}^{+\infty} \\
& = -\frac{\mathrm{erf}(-m^*/\sqrt{2\Delta})}{2} + (-1) + 1 - \frac{\mathrm{erf}(-m^*/\sqrt{2\Delta})}{2} \\
...
let
	...
	@progress for (k, Δ) in enumerate(Δ_values)
		...
		for _ in 1:nb_samples
			...
			m_mf[k] += mean(S) / nb_samples
			e_mf[k] += energy(S, h) / (N * nb_samples)
		end

		m_candidates = fixed_points_zeroT(; Δ, tol=1e-5)
...
let
	...
	for t in 1:tmax
		...
		mean_field_step!(S, h)
		m_mf = mean(S)
		m_se = erf(m_se / sqrt(2Δ))

		m_mf_history[t+1] = m_mf
		m_se_history[t+1] = m_se
	end

	plot(xlabel="Algorithm step", ylabel="Magnetization")
	plot!(0:tmax, m_se_history; lw=2, label="state evolution")
	scatter!(0:tmax, m_mf_history; label="mean field")
end
